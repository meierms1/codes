...
//#endif
	...
	ofstream pack3d ("pack3d.out");
	for (i=0; i<Particles.N; i++){
	  pack3d << Save.cx(i) << " " << Save.cy(i) << " " << Save.cz(i) << " " << Save.Radius(i)  << endl ;	  

	}
	ofstream pack2d ("pack2d.out");
        for (i=0; i<Particles.N; i++){
	  pack2d << Save.cx(i) << " " << Save.cy(i) << " " << Save.cz(i) << " " << sqrt(fabs(Save.Radius(i)*Save.Radius(i) - Save.cz(i) * Save.cz(i) )) << endl ;
